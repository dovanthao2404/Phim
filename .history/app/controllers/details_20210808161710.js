import { getData } from "./data.js";
let listFilm = document.querySelector(".film-connect .list-movie");
let filmBanner = document.querySelector(".film-img img");
let name = document.querySelector(".film-details h1");
let linkFilm = document.querySelector(".film-details a");

const renderCurrentFilm = () => {
  getData()
    .then(function (resp) {
      return resp.json();
    })
    .then(function (data) {
      filmBanner.setAttribute("src", data.phim.phimbo[1].imageUrl);
      name.innerText = data.phim.phimbo[1].title;
      linkFilm.setAttribute("href", data.phim.phimbo[1].url);
    })
}

const renderContact = () => {
  getData()
    .then(function (resp) {
      return resp.json();
    })
    .then(function (data) {

      for (let loaiPhim in data.phim) {
        let html = ""
        for (let i = 0; i < 6 * 3; i++) {
          html += `
        <div class="movie__item">
            <div class="movie__content">
            <a href="${data.phim[loaiPhim][i].url}">
              <img
                src="${data.phim[loaiPhim][i].imageUrl}"
                alt="">
              <p>${data.phim[loaiPhim][i].title}</p>
              </a>
            </div>
          </div>`
        }
        listFilm.innerHTML = html;
      }
    })
    .catch(function (err) {
      console.log(err)
    })
}



renderContact();
renderCurrentFilm();